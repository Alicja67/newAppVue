...
#
  echo "\n"
  echo " * user creation\n"
  curl -v https://sso.mac.pl:8008/auth/admin/realms/MAC-realm/users -H "Content-Type: application/json" -H "Authorization: bearer $TOKEN"   --data '{"username" : "'$field2'", "enabled": true, "email" : "'$field1'", "firstName": "'$field3'", "lastName": "'$field4'","credentials" : [{ "type" : "password", "value" : "MACinit!" ,"temporary":true} ],"realmRoles": [ "user", "offline_access"  ], "clientRoles": {"account": ["manage-account" ],"groups":["test1","test2"] } }'
  USER=`curl -v https://sso.mac.pl:8008/auth/admin/realms/MAC-realm/users?email=$field1 -H "Content-Type: application/json" -H "Authorization: bearer $TOKEN"`
  ID=`echo $USER | sed 's/.*id":"//g' | sed 's/".*//g'`
  echo "ID $ID"
  curl -v -X DELETE https://sso.mac.pl:8008/auth/admin/realms/MAC-realm/users/$ID/users/$ID -H "Content-Type: application/json" -H "Authorization: bearer $TOKEN"
done < data.csv
